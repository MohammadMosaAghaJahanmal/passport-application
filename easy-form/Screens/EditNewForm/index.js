...
import {View, Text, StyleSheet, Alert, TouchableOpacity, BackHandler, ScrollView} from 'react-native';
...
import { updateForm } from '../../DB';
import useStore from '../../store/store';
import { updateOne} from '../../utils/lightCrud';
import validator from '../../validators/application'
const EditNewForm = (props) =>
{
  ...
  const saveFormHandler = async () => 
  {
    ...
    let {data, message} = validator(fields)
    if(message)
    ...
    try {
      ...
      delete data.id
      await updateForm({...data, uxOldSerial: props.data.uxSerial});
      dispatch('setData', {type: "newforms", data: [...updateOne(newforms, props.data.uxSerial, fields, "uxSerial")]});
